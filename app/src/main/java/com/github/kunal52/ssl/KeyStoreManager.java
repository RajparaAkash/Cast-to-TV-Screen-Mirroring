...
public final class KeyStoreManager {
    ...
    private KeyStore createIdentityKeyStore() throws GeneralSecurityException {
        ...
        if (!useAndroidKeyStore()) {
            ...
            try {
                keyStore.load(null, this.androidRemoteContext.getKeyStorePass());
            } catch (IOException e) {
    ...
    private KeyStore load() {
        ...
        try {
            if (!useAndroidKeyStore()) {
                keyStore = KeyStore.getInstance(KeyStore.getDefaultType());
                keyStore.load(this.mContext.openFileInput(this.androidRemoteContext.getKeyStoreFileName()), this.androidRemoteContext.getKeyStorePass());
            } else {
                keyStore = KeyStore.getInstance(ANDROID_KEYSTORE);
                keyStore.load(null);
            }
    ...
    private void store(KeyStore keyStore) {
        if (!useAndroidKeyStore()) {
            try {
                FileOutputStream fileOutputStream = new FileOutputStream(this.androidRemoteContext.getKeyStoreFileName());
                keyStore.store(fileOutputStream, this.androidRemoteContext.getKeyStorePass());
